...
- **Supabase schema** lives in `api/schema.sql`, which you run by hand in the Supabase SQL editor. The tables are `profiles`, `subscriptions`, `payments`, `waitlist`, `exports` (monthly counts, claimed through the `claim_export` RPC), and `issues` (saved issues, plus the private `issues` storage bucket). RLS is on for all of them. `waitlist` has no policies, so only the service-role key can reach it.
